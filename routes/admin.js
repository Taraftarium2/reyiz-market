...
router.get('/', requireAdmin, async (req, res) => {
  ...
  const msg = req.query.msg && !Array.isArray(req.query.msg) ? req.query.msg : '';
  ...
  try {
    ...
    res.render('admin', { games, orders, users, coupons, stats, msg });
    return;
  } catch (err) {
...
router.post('/oyun', requireAdmin, upload.single('file'), async (req, res) => {
  ...
  if (!title) return res.redirect('/admin?msg=' + encodeURIComponent('Oyun başlığı zorunludur.'));
  const price = Number(req.body.price);
  if (!Number.isFinite(price) || price < 0) return res.redirect('/admin?msg=' + encodeURIComponent('Geçerli bir fiyat girin.'));
  const slug = (req.body.slug && req.body.slug.trim()) ? slugify(req.body.slug) : slugify(title);
  if (!slug) return res.redirect('/admin?msg=' + encodeURIComponent('Geçerli bir slug üretilemedi.'));

  try {
    const dup = (await db.query('SELECT id FROM games WHERE slug=$1', [slug])).rows[0];
    if (dup) return res.redirect('/admin?msg=' + encodeURIComponent('Bu slug zaten kullanımda: ' + slug + ' — başka bir slug girin.'));
    ...
    const r = await db.query(
      ...
      [title, slug, (req.body.description || '').trim(), price, (req.body.cover_image_url || '').trim(), file_key, (req.body.node_version || '18').trim(), (req.body.tag || 'Mini Oyun').trim(), req.body.featured ? true : false, (req.body.external_buy_url || '').trim()]
    );
    ...
    return res.redirect('/admin?msg=' + encodeURIComponent(title + ' başarıyla eklendi.'));
  } catch (e) {
    console.error('Oyun ekleme hatası:', e.message);
    return res.redirect('/admin?msg=' + encodeURIComponent('Oyun eklenirken hata oluştu:' + e.message));
  }
});
...
router.post('/oyun/:id/duzenle', requireAdmin, upload.single('file'), async (req, res) => {
  ...
  if (!Number.isInteger(id)) return res.redirect('/admin?msg=' + encodeURIComponent('Geçersiz oyun id.'));
  const title = (req.body.title || '').trim();
  const price = Number(req.body.price);
  if (!title || !Number.isFinite(price) || price < 0) {
    return res.redirect('/admin?msg=' + encodeURIComponent('Başlık ve geçerli bir fiyat zorunludur.'));
  }
  const slug = (req.body.slug && req.body.slug.trim()) ? slugify(req.body.slug) : slugify(title);
  const file_key = req.file ? req.file.filename : undefined;

  try {
    const dup = (await db.query('SELECT id FROM games WHERE slug=$1 AND id != $2', [slug, id])).rows[0];
    if (dup) return res.redirect('/admin?msg=' + encodeURIComponent('Bu slug başka bir oyun tarafından kullanılıyor.'));
    ...
    const params = [title, slug, (req.body.description || '').trim(), price, (req.body.cover_image_url || '').trim(), (req.body.node_version || '18').trim(), (req.body.tag || 'Mini Oyun').trim(), req.body.featured ? true : false, (req.body.external_buy_url || '').trim()];
