...
const COOKIE_OPTS = { httpOnly: false, sameSite: 'lax', path: '/', secure: COOKIE_SECURE, maxAge: 7 * 24 * 3600 * 1000 };

function readCart(req) {
  try {
    const raw = JSON.parse(req.cookies.cart || '[]');
    return Array.isArray(raw) ? raw.filter((i) => Number.isInteger(Number(i)) && Number(i) > 0).map(Number) : [];
  } catch (e) { return []; }
}
function writeCart(res, ids) { res.cookie('cart', JSON.stringify(ids), COOKIE_OPTS); }

function readCoupon(req) { try { return JSON.parse(req.cookies.coupon || 'null'); } catch (e) { return null; } }
function writeCoupon(res, coupon) { res.cookie('coupon', JSON.stringify(coupon), COOKIE_OPTS); }
function clearCoupon(res) { res.clearCookie('coupon', { path: '/' }); }
...
router.post('/sepet/ekle', async (req, res) => {
  ...
  const jsonReq = req.xhr || (req.headers['accept'] && req.headers['accept'].includes('json')) || req.headers['x-requested-with'] === 'XMLHttpRequest';

  if (!Number.isInteger(id) || id <= 0) {
    if (jsonReq) return res.status(400).json({ success: false, error: 'Geçersiz oyun id.' });
    return res.redirect('/oyunlar');
  }

  // Oyunun gerçekten var olduğunu doğrula (sahte ürün ilave edilemesin)
  const exists = (await db.query('SELECT id FROM games WHERE id=$1', [id])).rows.length > 0;
  if (!exists) {
    if (jsonReq) return res.status(404).json({ success: false, error: 'Oyun bulunamadı.' });
    return res.redirect('/oyunlar');
  }
...
router.post('/odeme', requireAuth, async (req, res) => {
  ...
  if (giftEmail) {
    if (!giftEmail.includes('@')) {
      giftError = 'Geçerli bir hediye e-postası girin.';
    } else {
      const uRes = await db.query('SELECT id FROM users WHERE LOWER(email)=$1', [giftEmail]).catch(() => null);
      if (uRes && uRes.rows[0]) targetUserId = uRes.rows[0].id;
      else giftError = 'Bu e-posta adresine sahip bir hesap bulunamadı. Alıcının önce kayıt olması gerekir.';
    }
  }
  if (giftError) {
    return res.render('checkout', { items: games, subtotal, discount, total, coupon, error: giftError });
  }
  ...
  try {
    ...
    if (mode === 'mock') {
      await finalizeOrder(targetUserId, games, total, 'mock', 'mock_' + Date.now(), coupon, discount);
      writeCart(res, []);
      clearCoupon(res);
    ...
    const orderId = await createPendingOrder(targetUserId, games, total, 'manual', ref, coupon, discount);
    writeCart(res, []);
    clearCoupon(res);
    res.redirect('/odeme/beklemede/' + orderId);
  } catch (e) {
    console.error('Ödeme başlatma hatası:', e);
    return res.render('checkout', { items: games, subtotal, discount, total, coupon, error: 'Sipariş oluşturulamadı. Lütfen tekrar deneyin.' });
  }
});
...
router.post('/odeme/beklemede/:id/hizli-onay', requireAdmin, async (req, res) => {
  const orderId = Number(req.params.id);
...
router.get('/odeme/sonuc', requireAuth, async (req, res) => {
  ...
  if (ok) {
    ...
    const coupon = readCoupon(req);
    const { subtotal, discount, total } = calculateCartTotals(games, coupon);
    await finalizeOrder(req.user.id, games, total, 'iyzico', token, coupon, discount);
    writeCart(res, []);
    clearCoupon(res);
    return res.redirect('/odeme/basarili');
  }
...
async function createPendingOrder(userId, games, total, provider, ref, coupon, discount) {
  const cli = await db.connect();
  try {
    await cli.query('BEGIN');
    const o = await cli.query(
      `INSERT INTO orders (user_id, total_amount, status, payment_provider, payment_ref, coupon_code, discount_amount)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
      [userId, total, 'pending', provider, ref, coupon ? coupon.code : null, discount || 0]
    );
...
async function finalizeOrder(userId, games, total, provider, ref, coupon, discount) {
  const cli = await db.connect();
  try {
    await cli.query('BEGIN');
    const o = await cli.query(
      `INSERT INTO orders (user_id, total_amount, status, payment_provider, payment_ref, coupon_code, discount_amount)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
      [userId, total, 'paid', provider, ref, coupon ? coupon.code : null, discount || 0]
    );
    const orderId = o.rows[0].id;
    for (const g of games) {
      await cli.query('INSERT INTO order_items (order_id, game_id, price_at_purchase) VALUES ($1,$2,$3)', [orderId, g.id, g.price]);
